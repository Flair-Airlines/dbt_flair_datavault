...
from {{ ref('VW_STG_D_GL_PAYMENTS') }} t1
--from FIVETRAN_DATABASE.PSS_AMELIARES_DBO.TBL_GL_PAYMENTS t1
left join {{ ref('VW_STG_D_GL_PAYMENT_METHOD') }} t2 on t2.LNG_GL_PAYMENT_METHOD_ID_NMBR = t1.LNG_GL_PAYMENT_METHOD_ID_NMBR
left join {{ ref('VW_STG_D_CURRENCY') }} t3 on t3.LNG_CURRENCY_ID_NMBR = t1.LNG_CURRENCY_ID_NMBR
left join {{ ref('VW_STG_D_RES_HEADER') }} t4 on t4.LNG_RESERVATION_NMBR = t1.LNG_RESERVATION_NMBR
--where "Reservation Nmbr" = 7745460