...
LEFT JOIN {{ source('PSS_AMELIARES_DBO', 'TBL_RES_PAX_GROUP') }} t2 on t2.LNG_RESERVATION_NMBR=t1.LNG_RESERVATION_NMBR
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_RES_SEGMENTS') }}  t3 on (t3.LNG_RES_LEGS_ID_NMBR = t1.LNG_RES_LEGS_ID_NMBR and CAST(t1.DTM_GL_CHARGES_DATE AS DATE) = CAST(t3.DTM_CREATION_DATE AS DATE))
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_SKED_DETAIL') }} t4 on t4.LNG_SKED_DETAIL_ID_NMBR = t3.LNG_SKED_DETAIL_ID_NMBR
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_AIRPORT') }} t5 on t5.LNG_AIRPORT_ID_NMBR = t4.LNG_DEP_AIRPORT_ID_NMBR
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_AIRPORT') }} t6 on t6.LNG_AIRPORT_ID_NMBR = t4.LNG_ARR_AIRPORT_ID_NMBR
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_SKED_FLIGHT_WATCH_JOURNEY_LOG') }}  t7 on t7.LNG_SKED_DETAIL_ID_NMBR = t4.LNG_SKED_DETAIL_ID_NMBR
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_RES_HEADER') }} t8 on t8.LNG_RESERVATION_NMBR = t1.LNG_RESERVATION_NMBR
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_AGENCY') }} t9 on t9.LNG_AGENCY_ID_NMBR = t8.LNG_AGENCY_ID_NMBR
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_RES_CHECKIN') }} t10 on t10.LNG_RES_LEGS_ID_NMBR = t1.LNG_RES_LEGS_ID_NMBR
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_PAX') }} t11 on t11.LNG_PAX_ID_NMBR = t2.LNG_PAX_ID_NMBR 
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_RES_LEGS') }} t12 ON t12.LNG_RES_LEGS_ID_NMBR = t1.LNG_RES_LEGS_ID_NMBR
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_USERS') }} t13 ON t1.LNG_CREATION_USER_ID_NMBR  = t13.lng_User_Id_Nmbr
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_PROVINCE_DEFINITION') }}  AS t14 ON t5.lng_Province_Id_Nmbr = t14.lng_Province_Id_Nmbr
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_PROVINCE_DEFINITION') }} AS t15 ON t6.lng_Province_Id_Nmbr = t15.lng_Province_Id_Nmbr
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_COUNTRY') }}  AS t16 ON t16.LNG_COUNTRY_ID_NMBR = t14.LNG_COUNTRY_ID_NMBR 
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_COUNTRY') }} AS t17 ON t17.LNG_COUNTRY_ID_NMBR = t15.LNG_COUNTRY_ID_NMBR
LEFT JOIN  {{ source('PSS_AMELIARES_DBO', 'TBL_USERS') }} t18 ON t10.LNG_CREATION_USER_ID_NMBR  = t18.lng_User_Id_Nmbr