...
{% if is_incremental() %}
  ...
  where NOT EXISTS (SELECT 1 FROM {{ this }} AS Y WHERE X.PRODUCT_CODE = Y.PRODUCT_CODE AND X.PRODUCT_GROUP = Y.PRODUCT_GROUP)
--INSERTDATETIME > IFNULL((select max(INSERTDATETIME) from {#{{ this }}#}),'1900-01-01')
